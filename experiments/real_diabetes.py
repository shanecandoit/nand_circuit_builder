"""
Diabetes Regression Experiment

Tests CircuitBuilder on the sklearn diabetes dataset (regression task).
- 10 features → 50 binary inputs (5 buckets each)
- Target: Disease progression (continuous)
- Success metric: R² score > 0.4
"""
...
def test_diabetes():
    """Test learning diabetes disease progression."""
    print("\n" + "="*70)
    print("DIABETES REGRESSION: Disease Progression Prediction")
    print("="*70)
    ...
    # Model configuration: 10 features × 5 buckets = 50 binary inputs
    # Use more gates for complex real-world data
    model = CircuitBuilder(
        n_gates=50,  # More gates for complex relationships
        input_buckets=5,  # 5 buckets per feature = 50 total binary inputs
        random_state=42,
        learning_rate=0.01,
    ...
    print(f"\n  Target: R² > 0.4")
    print(f"  Status: {'[PASS]' if test_r2 > 0.4 else '[FAIL]'}")
    ...
    for i in range(min(10, len(y_test_orig))):
        ...
        print(f"{y_test_orig[i]:<12.1f}{y_test_pred_orig[i]:<12.1f}{error:<12.1f}{error_pct:<12.1f}")
    
    # Create combined plot with loss and R²
    # For R² history, we'll approximate by computing it at key epochs
    # using the final model (shows trend, not exact historical values)
...
def create_loss_r2_plot(history, r2_history, final_r2):
    ...
    # Add target line at R² = 0.4
    ax2.axhline(y=0.4, color='r', linestyle='--', linewidth=2, 
               label='Target (0.4)', alpha=0.7, zorder=0)
    ...
    # Overall title
    fig.suptitle('Diabetes Regression: Loss and R² Score\n50 gates, 50 binary inputs (10 features × 5 buckets)', 
                fontsize=16, fontweight='bold')
    
    plt.tight_layout()
    
    # Save plot
    plot_file = 'outputs/diabetes_regression_loss_r2.png'
...
if __name__ == "__main__":
    print("Diabetes Regression Experiment")
    print("="*70)
    print("Testing CircuitBuilder on sklearn diabetes dataset")
    print("Target: R² score > 0.4\n")
    
    success = test_diabetes()
    
    print(f"\n{'='*70}")
    print("EXPERIMENT SUMMARY")
    print('='*70)
    print(f"Result: {'[PASS]' if success else '[FAIL]'}")
    print(f"Target metric: R² > 0.4")
